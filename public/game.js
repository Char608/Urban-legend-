...
// Canvas grabbed lazily on first use (safe across all load orders)
...
// ── Join game ─────────────────────────────────────────────────
function _startGame(skinIndex, gameMode) {
  const name = (document.getElementById('name-in').value||'').trim().toUpperCase() || 'PLAYER';
  if (skinIndex !== undefined) selectedSkin = skinIndex;
  if (gameMode  !== undefined) Game.selectedMode = gameMode;
...
// joinGame() is defined in index.html inline boot script — do not redefine here.
